...
window.addEventListener("load", function () { return generateDivs(containerDiv); });
var ROWS = 4;
var TILES_IN_ROWS = 4;
...
function getDivInvertedColor(div) {
    var theCSSprop = window
        .getComputedStyle(div, null)
        .getPropertyValue("background-color");
    var color = rgb2hex(theCSSprop);
...
function handleTileClick(e) {
    ...
    var allColsDivs = document.querySelectorAll("[data-column=\"" + clickedCol + "\"]");
    allRowsDivs.forEach(function (div, i) {
        if (i != parseInt(clickedCol)) {
            var timeDelay = Math.abs(parseInt(clickedCol) - i);
            setTimeout(function () {
